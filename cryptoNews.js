    ...
    const POLYGON_NEWS_API_URL = 'https://min-api.cryptocompare.com/data/v2/news/?lang=EN&api_key=4dae1cb54d65bcf2c1cf6a4937bca72034b90f16b6a6be46537a8831e3fb605b';
    ...
    async function fetchNews() {
        ...
        try {
            ...
            // 🛑 FIX: Access 'Data' property and use CryptoCompare's object keys 🛑
            const articles = (data.Data || []).map((a, i) => ({
                id: a.guid || i, // Use 'guid' for a unique ID
                title: a.title, // 'title' is correct
                url: a.url, // 'url' is correct
                image: a.imageurl, // Use 'imageurl' from CryptoCompare
            }));

            logger.log(`Fetched ${articles.length} new articles.`).log(articles[0].title); 
